...
/* === CSV Export / Import === */
function exportGirlsCSV(){
  const cols=['name','country','age','body','height','cup','val1','val2','val3','special','exp','startDate','lang','type','desc','labels','hidden','photos'];
  const q=s=>`"${String(s||'').replace(/"/g,'""')}"`;
  const rows=[cols.join(',')];
  girls.forEach(g=>{rows.push([q(g.name),q(Array.isArray(g.country)?g.country.join(';'):g.country||''),q(g.age),q(g.body),q(g.height),q(g.cup),q(g.val1),q(g.val2),q(g.val3),q(g.special),q(g.exp),q(g.startDate),q(g.lang),q(g.type),q(g.desc),q(Array.isArray(g.labels)?g.labels.join(';'):g.labels||''),q(g.hidden?'1':''),q(Array.isArray(g.photos)?g.photos.join(';'):g.photos||'')].join(','))});
  const blob=new Blob([rows.join('\n')],{type:'text/csv;charset=utf-8;'});
  const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='ginza-profiles-'+fmtDate(getAEDTDate())+'.csv';a.click();URL.revokeObjectURL(a.href);
}

function _parseCSV(text){
  const rows=[];let row=[],cell='',inQ=false;
  for(let i=0;i<text.length;i++){const ch=text[i];if(inQ){if(ch==='"'&&text[i+1]==='"'){cell+='"';i++}else if(ch==='"')inQ=false;else cell+=ch}else{if(ch==='"')inQ=true;else if(ch===','){row.push(cell);cell=''}else if(ch==='\n'||(ch==='\r'&&text[i+1]==='\n')){if(ch==='\r')i++;row.push(cell);cell='';rows.push(row);row=[]}else cell+=ch}}
  if(cell||row.length){row.push(cell);rows.push(row)}
  return rows;
}

function importGirlsCSV(csvText){
  ...
  const toAdd=[],toUpdate=[];let unchanged=0;
  const parsed=rows.slice(1).filter(r=>r.some(c=>c.trim())).map(row=>{const name=get(row,'name');if(!name)return null;return{name,country:get(row,'country').split(';').filter(Boolean),age:get(row,'age'),body:get(row,'body'),height:get(row,'height'),cup:get(row,'cup'),val1:get(row,'val1'),val2:get(row,'val2'),val3:get(row,'val3'),special:get(row,'special'),exp:get(row,'exp'),startDate:get(row,'startDate'),lang:get(row,'lang'),type:get(row,'type'),desc:get(row,'desc'),labels:get(row,'labels').split(';').filter(Boolean),hidden:get(row,'hidden')==='1',_photosRaw:get(row,'photos')}}).filter(Boolean);
  parsed.forEach(p=>{
    const existIdx=girls.findIndex(g=>(g.name||'').trim().toLowerCase()===(p.name||'').trim().toLowerCase());
    if(existIdx<0){toAdd.push(p)}else{const g=girls[existIdx];const changed=['age','body','height','cup','val1','val2','val3','special','exp','startDate','lang','type','desc'].some(k=>String(p[k]||'')!==String(g[k]||''))||JSON.stringify(p.country)!==JSON.stringify(Array.isArray(g.country)?g.country:[g.country||''].filter(Boolean))||JSON.stringify(p.labels)!==JSON.stringify(Array.isArray(g.labels)?g.labels:[])||p.hidden!==!!g.hidden;if(changed)toUpdate.push({p,existIdx});else unchanged++}
  });
  const overlay=document.getElementById('csvImportOverlay');
  const summary=document.getElementById('csvImportSummary');
  let html=`<div style="margin-bottom:16px;line-height:2"><div><b style="color:var(--accent)">${toAdd.length}</b> profiles to add</div><div><b style="color:#ffcc44">${toUpdate.length}</b> profiles to update</div><div style="color:var(--text-dim)">${unchanged} unchanged</div></div>`;
  if(toUpdate.length){html+='<div style="font-size:12px;color:var(--text-dim);margin-bottom:4px">Profiles being updated:</div><div style="font-size:12px;max-height:100px;overflow-y:auto;margin-bottom:8px">';toUpdate.forEach(({p})=>{html+=`<div style="padding:2px 0;border-bottom:1px solid rgba(255,255,255,0.06)">${p.name}</div>`});html+='</div>'}
  summary.innerHTML=html;
  const confirmBtn=document.getElementById('csvImportConfirm');
  confirmBtn.onclick=async()=>{
    confirmBtn.textContent='Applying...';confirmBtn.style.pointerEvents='none';
    try{
      toUpdate.forEach(({p,existIdx})=>{const g=girls[existIdx];const photos=p._photosRaw?p._photosRaw.split(';').filter(Boolean):g.photos||[];girls[existIdx]={...g,...p,photos,labelsJa:p.labels,lastModified:new Date().toISOString()};delete girls[existIdx]._photosRaw});
      toAdd.forEach(p=>{const photos=p._photosRaw?p._photosRaw.split(';').filter(Boolean):[];girls.push({...p,photos,location:'Empire',labelsJa:p.labels,lastModified:new Date().toISOString()});delete girls[girls.length-1]._photosRaw});
      if(await saveData()){overlay.classList.remove('open');renderFilters();renderGrid();renderRoster();renderHome();showToast(`Import done: ${toAdd.length} added, ${toUpdate.length} updated`);logAdminAction('csv_import',`${toAdd.length} added / ${toUpdate.length} updated`)}
    }catch(e){showToast('Import failed: '+e.message,'error')}
    finally{confirmBtn.textContent='Apply Changes';confirmBtn.style.pointerEvents='auto'}
  };
