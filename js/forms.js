...
document.addEventListener('DOMContentLoaded',()=>{const inp=document.getElementById('fLabelInput');if(inp){inp.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();const v=sanitize(inp.value);if(v&&!formLabels.includes(v)){formLabels.push(v);renderFormLabels()}inp.value=''}})}});
...
else{document.getElementById('formTitle').textContent='Add New Girl';formFields.forEach(id=>document.getElementById(id).value='');document.getElementById('fStartDate').value=fmtDate(getAEDTDate());formPhotos=[];formLabels=[];formCountries=[]}renderFormPhotos();renderFormLabels();renderFormCountries();formOverlay.classList.add('open')}
