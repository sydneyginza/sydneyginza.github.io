...
const formFields=['fName','fAge','fBody','fHeight','fCup','fVal1','fVal2','fVal3','fSpecial','fExp','fStartDate','fLang','fType','fDesc','fSpecialJa','fLangJa','fTypeJa','fDescJa'];
let formLabels=[];
let formLabelsJa=[];
...
function renderFormLabelsJa(){const c=document.getElementById('fLabelTagsJa');if(!c)return;c.innerHTML='';
formLabelsJa.forEach((lbl,i)=>{const tag=document.createElement('span');tag.className='label-tag';tag.innerHTML=`${lbl}<button class="label-remove" title="Remove">&times;</button>`;tag.querySelector('.label-remove').onclick=()=>{formLabelsJa.splice(i,1);renderFormLabelsJa()};c.appendChild(tag)})}
...
const inpJa=document.getElementById('fLabelInputJa');if(inpJa){inpJa.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();const v=sanitize(inpJa.value);if(v&&!formLabelsJa.includes(v)){formLabelsJa.push(v);renderFormLabelsJa()}inpJa.value=''}})}
});
...
if(idx>=0){document.getElementById('formTitle').textContent='Edit Profile';const g=girls[idx];document.getElementById('fName').value=g.name;const gc=g.country||'';formCountries=Array.isArray(gc)?[...gc]:(gc?[gc]:[]);document.getElementById('fAge').value=g.age;document.getElementById('fBody').value=g.body;document.getElementById('fHeight').value=g.height;document.getElementById('fCup').value=g.cup;document.getElementById('fVal1').value=g.val1||'';document.getElementById('fVal2').value=g.val2||'';document.getElementById('fVal3').value=g.val3||'';document.getElementById('fSpecial').value=g.special||'';document.getElementById('fExp').value=g.exp||'';document.getElementById('fStartDate').value=g.startDate||fmtDate(getAEDTDate());document.getElementById('fLang').value=g.lang||'';document.getElementById('fType').value=g.type||'';document.getElementById('fDesc').value=g.desc;document.getElementById('fSpecialJa').value=g.specialJa||'';document.getElementById('fLangJa').value=g.langJa||'';document.getElementById('fTypeJa').value=g.typeJa||'';document.getElementById('fDescJa').value=g.descJa||'';formPhotos=[...(g.photos||[])];formLabels=[...(g.labels||[])];formLabelsJa=[...(g.labelsJa||[])]}
else{document.getElementById('formTitle').textContent='Add New Girl';formFields.forEach(id=>document.getElementById(id).value='');document.getElementById('fStartDate').value=fmtDate(getAEDTDate());formPhotos=[];formLabels=[];formLabelsJa=[];formCountries=[]}renderFormPhotos();renderFormLabels();renderFormLabelsJa();renderFormCountries();formOverlay.classList.add('open')}
