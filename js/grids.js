...
const _rat=g.name?getAverageRating(g.name):{avg:0,count:0};const _ratHtml=_rat.count>0?`<div class="card-rating">${renderStars(_rat.avg,_rat.count)}</div>`:'';
el.innerHTML=`<div class="card-img" style="background:linear-gradient(135deg,rgba(180,74,255,0.06),rgba(255,111,0,0.03))">${img}${fav}${cmp}${act}</div><div class="card-info"><div class="card-name">${g.name||''}</div><div class="card-country">${Array.isArray(g.country)?g.country.join(', '):(g.country||'')}</div>${avail||schedLabel}${_ratHtml}${loggedIn&&g.lastModified?`<div style="font-size:10px;color:rgba(255,255,255,0.28);letter-spacing:1px;margin-top:2px">${daysAgo(g.lastModified)}</div>`:''}<div class="card-hover-line"></div></div>`;
...
const fav=g.name?cardFavBtn(g.name):'';const cmp=g.name?cardCompareBtn(g.name):'';
const _rrat=g.name?getAverageRating(g.name):{avg:0,count:0};const _rratHtml=_rrat.count>0?`<div class="card-rating">${renderStars(_rrat.avg,_rrat.count)}</div>`:'';
el.innerHTML=`<div class="card-img" style="background:linear-gradient(135deg,rgba(180,74,255,0.06),rgba(255,111,0,0.03))">${img}${fav}${cmp}</div><div class="card-info"><div class="card-name">${g.name||''}</div><div class="card-country">${Array.isArray(g.country)?g.country.join(', '):(g.country||'')}</div>${avail}${_rratHtml}<div class="card-hover-line"></div></div>`;
...
const _frat=g.name?getAverageRating(g.name):{avg:0,count:0};const _fratHtml=_frat.count>0?`<div class="card-rating">${renderStars(_frat.avg,_frat.count)}</div>`:'';
el.innerHTML=`<div class="card-img" style="background:linear-gradient(135deg,rgba(180,74,255,0.06),rgba(255,111,0,0.03))">${img}${fav}${cmp}</div><div class="card-info"><div class="card-name">${g.name||''}</div><div class="card-country">${Array.isArray(g.country)?g.country.join(', '):(g.country||'')}</div>${avail}${_fratHtml}<div class="card-hover-line"></div></div>`;
