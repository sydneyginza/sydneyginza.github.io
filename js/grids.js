...
const fg=(calLocFilter==='All'?girls:girls.filter(g=>g.location===calLocFilter)).filter(g=>g.name&&g.name.trim());const table=document.getElementById('calTable');const dates=getWeekDates();const ts=dates[0];const tOpts=generateTimeOptions();
