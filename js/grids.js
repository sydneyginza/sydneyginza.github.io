...
const fg=(calLocFilter==='All'?girls:girls.filter(g=>g.location===calLocFilter)).filter(g=>g.name&&String(g.name).trim().length>0).sort((a,b)=>a.name.localeCompare(b.name));const table=document.getElementById('calTable');const dates=getWeekDates();const ts=dates[0];const tOpts=generateTimeOptions();
