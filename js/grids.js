...
table.querySelectorAll('.cal-clear-day-btn').forEach(btn=>{btn.onclick=e=>{e.stopPropagation();const ds=btn.dataset.clearDate;const f=dispDate(ds);if(!confirm(`Clear all girls for ${f.day} ${f.date}?`))return;let cleared=0;for(const name in calData){if(name==='_published')continue;if(calData[name]&&calData[name][ds]){delete calData[name][ds];cleared++;if(calPending[name])delete calPending[name][ds]}}if(cleared>0){queueCalSave(null,100);renderCalendar();renderRoster();renderGrid();renderHome();showToast(`Cleared ${cleared} entries for ${f.day} ${f.date}`)}else{showToast('No entries to clear')}}})}
